...
from convert.convert_mhd_to_dicom import convert_mhd_to_dicom

DATA_DIR = os.path.join(BASE_DIR, "data", "subset0")
OUTPUT_DIR = os.path.join(BASE_DIR, "data", "dicom")

def main():
    image_files = sorted([os.path.join(DATA_DIR, f) for f in os.listdir(DATA_DIR) if f.endswith('.mhd')])
